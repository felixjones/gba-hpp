...
#ifndef GBAXX_EXT_AGBABI_DETAIL_CORO_CONTEXT_HPP
#define GBAXX_EXT_AGBABI_DETAIL_CORO_CONTEXT_HPP

#include <agbabi.h>

namespace gba::agbabi::detail {

    using coro_proc_type = int(*)(__agbabi_coro_t*);

    template <typename T>
    struct context_type;

    template <typename T>
    struct coro_type : __agbabi_coro_t {
        context_type<T>* ctx;
        T* value;
    };

    template <>
    struct coro_type<void> : __agbabi_coro_t {
...
#endif // define GBAXX_EXT_AGBABI_DETAIL_CORO_CONTEXT_HPP
