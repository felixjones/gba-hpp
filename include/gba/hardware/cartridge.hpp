 ...
 * </table><br />
 *
 * When prefetch is enabled, the GBA attempts to read opcodes from Game Pak ROM during periods when the CPU is not
 * using the bus. Memory access is then performed with 0 Waits if the CPU requests data which is already stored in the
 * buffer. The prefetch buffer stores up to eight 16bit values.
 *
 * @sa mmio::WAITCNT
 */
